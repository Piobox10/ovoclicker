...
const EmbryoEquipmentPanel: React.FC<EmbryoEquipmentPanelProps> = ({ equippedItems, allItemsData, onOpenInventoryModal, onUnequipItem }) => {
  const slots: EmbryoEquipmentSlotKey[] = ['weapon', 'armor', 'passiveChip'];
  ...
  const getEquippedItemDetails = (itemId: string | null): EmbryoItem | null => {
    if (!itemId) return null;
    return allItemsData.find(item => item.id === itemId) || null;
  };
    ...
    <CollapsibleSection title="Equipamento do Embrião" titleIcon="fas fa-user-shield" initiallyOpen={false}>
      <div className="space-y-3">
        {slots.map(slotKey => {
          ...
          const itemDetails = getEquippedItemDetails(equippedItemId);
              ...
              {itemDetails ? (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`${rarityStyle.text} text-xs`}>{itemDetails.name}</span>
                  </div>
                  <button 
