...
import { GameState, EggStage } from '../../types';
import { formatNumber, formatTime, getEmbryoVisuals } from '../../utils';

interface StatusDisplayPanelProps {
  gameState: Pick<GameState, 
    | 'incubationPower' | 'modularEXP' | 'temporaryEggs' | 'effectiveClicksPerClick' 
    | 'effectiveIpps' | 'effectiveCriticalClickChance' | 'transcendentEssence' 
    | 'embryoLevel' | 'enemiesDefeatedTotal' | 'transcendenceCount' 
    | 'transcendencePassiveBonus' | 'userNickname' | 'transcendenceSpamPenaltyActive' 
    | 'transcendenceSpamPenaltyDuration'
  >;
  currentEggStage: EggStage;
  // className?: string; // Prop removida
}

const StatusDisplayPanel: React.FC<StatusDisplayPanelProps> = ({ gameState, currentEggStage }) => {
  return (
    <div className={`game-stats w-full bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-4 sm:p-6 border border-slate-700 text-slate-200 text-sm sm:text-base text-center shadow-lg`}>
      <p className="mb-1"><i className="fas fa-egg text-violet-400 mr-2 opacity-80"></i> PI: <span className="font-bold text-lg text-amber-300">{formatNumber(gameState.incubationPower)}</span></p>
      ...
      <p className="mb-1"><i className="fas fa-hand-pointer text-violet-400 mr-2 opacity-80"></i> PI/Clique: <span className="font-bold text-indigo-300">{formatNumber(gameState.effectiveClicksPerClick)}</span></p>
      <p className="mb-1"><i className="fas fa-hourglass-half text-violet-400 mr-2 opacity-80"></i> PI/Seg: <span className="font-bold text-indigo-300">{formatNumber(gameState.effectiveIpps)}</span></p>
      ...
      <p className="mb-1"><i className="fas fa-layer-group text-teal-400 mr-2 opacity-80"></i> Estágio: <span className="font-bold text-teal-300">{currentEggStage.name}</span></p>
      ...
      <p className="mt-2 text-xs text-slate-400">ID: <span className="font-semibold">{gameState.userNickname}</span></p>
